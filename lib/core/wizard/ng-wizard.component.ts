...
export class NgWizardComponent implements OnDestroy, AfterContentInit {
  ...
  _getPreviousButtonCssClass() {
    var prevButtonClass = this.styles.previousButton;

    if (this._pConfig.toolbarSettings.prevBtnCss) {
      prevButtonClass += ' ' + this._pConfig.toolbarSettings.prevBtnCss;
    }

    return prevButtonClass;
  }

  _getNextButtonCssClass() {
    var nextButtonClass = this.styles.nextButton;

    if (this._pConfig.toolbarSettings.nextBtnCss) {
      nextButtonClass += ' ' + this._pConfig.toolbarSettings.nextBtnCss;
    }
  ...
  _getBtnClass() {
    var btnClass = 'btn-group mr-2 ng-wizard-btn-group';

    if (this._pConfig.toolbarSettings.btnGroupCss) {
      btnClass += ' ' + this._pConfig.toolbarSettings.btnGroupCss;
    }
  ...
  _getStepCssClass(selectedStep: NgWizardStep) {
    ...
    if (selectedStep.customCss){
      stepClass += ' ' + selectedStep.customCss;
    }
